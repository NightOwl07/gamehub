...
namespace TTT.EntitySync
{
    ...
    public class PlayerLabel : Entity, IEntity
    {
        ...
        public PlayerLabel(Vector3 position, int dimension, uint range, ulong entityType) : base(entityType, position,
            dimension, range)
        {
        ...
        public bool Center
        {
            get
            {
                if (!this.TryGetData("center", out bool center))
                    return default;

                return center;
            }
            set => this.SetData("center", value);
        }
        ...
        public bool Proportional
        {
            get
            {
                if (!this.TryGetData("proportional", out bool proportional))
                    return default;

                return proportional;
            }
            set => this.SetData("proportional", value);
        }
        ...
        public Rgba Edge
        {
            get
            {
                if (!this.TryGetData("edge", out Dictionary<string, object> data))
                    return default;
            ...
            {
                // No data changed
                if (this.Edge != null && this.Edge.R == value.R && this.Edge.G == value.G && this.Edge.B == value.B &&
                    this.Edge.A == value.A)
                    return;
        ...
        public DropShadow DropShadow
        {
            get
            {
                if (!this.TryGetData("dropShadow", out Dictionary<string, object> data))
                    return null;
            ...
            {
                // No data changed
                if (this.DropShadow != null && this.DropShadow.Distance == value.Distance &&
                    this.DropShadow.R == value.R && this.DropShadow.G == value.G && this.DropShadow.B == value.B &&
                    this.DropShadow.A == value.A)
                    return;
        ...
        public int Font
        {
            get
            {
                if (!this.TryGetData("font", out int font))
                    return default;

                return font;
            }
            set => this.SetData("font", value);
        }
    ...
    public static class TextLabelStreamer
    {
        ...
        public static PlayerLabel Create(
            string text, Vector3 position, int dimension = 0, bool? center = true, Rgba? color = null,
            float? scale = 0.7f,
            DropShadow dropShadow = null, Rgba? edge = null, int? font = null, bool? proportional = null,
            uint streamRange = 45
        )
        {
            PlayerLabel textLabel = new(position, dimension, streamRange, 1)
            {
                Center = center ?? true,
        ...
        {
            foreach (PlayerLabel obj in GetAllDynamicTextLabels()) AltEntitySync.RemoveEntity(obj);
        ...
        {
            List<PlayerLabel> textLabels = new();
